...
Config lives in [`render.yaml`](render.yaml) rather than in a dashboard, so the
...
database lives on **TiDB Cloud Starter** — free tier, no credit card, and it speaks
the MySQL wire protocol, so application code, PyMySQL and MySQL Workbench all
connect unchanged.

The alternative free MySQL host (Aiven) was rejected on a specific risk: its free
tier *powers off after inactivity* and appears to need a manual restart. For a demo
a reviewer might open days later, that's a submission that silently dies. TiDB is
serverless — it scales to zero and wakes on connection, so it cannot become
unreachable while nobody is looking.

**TiDB is MySQL-*compatible*, not MySQL.** That distinction cost us something real
— see below.

### ⚠️ Required TiDB setting: `tidb_enable_check_constraint`

**TiDB does not enforce `CHECK` constraints by default.** It parses them and
silently ignores them. An `event_type` of `'banana'` that MySQL 8.4 refuses goes
straight into TiDB.

This is not theoretical — it was caught by
`test_check_constraint_rejects_unknown_event_type`, which passed on MySQL 8.4 and
**failed on TiDB**, before anything was deployed. Fix:

```sql
SET GLOBAL tidb_enable_check_constraint = ON;
```

Global and persistent, but it is **cluster state that lives nowhere in this repo**.
A freshly provisioned cluster will silently lose `CHECK` enforcement. This is a
genuine footgun of the MySQL-compatible-but-not-MySQL choice.

Two things keep it honest:
1. It's a documented step (here, and in the checklist below).
2. The test suite is the canary — run it against any new cluster and that test
   fails loudly if the setting is missing.

If it ever *were* missing, Pydantic still rejects unknown event types before they
reach the database. `CHECK` is defence in depth, not the only line — but "second
line of defence" is only true if it's actually there.

### First-time setup

**1. TiDB Cloud** — sign up at <https://tidbcloud.com>, create a **Starter**
cluster in `ap-southeast-1`, then:

```sql
CREATE DATABASE setu;
SET GLOBAL tidb_enable_check_constraint = ON;   -- see above. Not optional.
```
...
Loading takes ~15s over the public internet (vs 0.9s locally — that's ~14s of
round-trip latency to Singapore, not slow code). It's idempotent, so re-running is
harmless: every event reports as a duplicate and nothing changes.

**3. Render** — New → Blueprint → point it at this repo. `render.yaml` configures
everything except the one secret:

| env var | value |
|---|---|
| `DATABASE_URL` | the TiDB URL above — **set in the dashboard, never committed** |
...
**TLS is mandatory** — TiDB refuses plaintext, correctly: without it, credentials
and every row would cross the public internet in the clear. We verify against
`certifi`'s CA bundle rather than shipping a provider CA file: fewer moving parts,
nothing to expire, and it works against any managed MySQL. Verified live —
`Ssl_cipher = TLS_AES_128_GCM_SHA256`.

**`pool_pre_ping=True`** — managed databases drop idle connections. Without it the
pool hands out a socket the server already closed, and the first request after a
quiet period fails with *"MySQL server has gone away"*. On a free tier that sleeps,
that's not an edge case; it's every morning.

### Known limits of this deployment

- **Render free instances sleep after ~15 min idle.** The first request wakes it —
  expect **~30–50s** on a cold hit, then normal speed. If the demo seems dead, it's
  waking up; retry once.
- **Every query crosses Render → TiDB over the internet.** Both are in Singapore to
  keep that hop short, but it's still a network round trip where local Docker had
  none. Fine for a demo, and the reason `docker compose up` is faster than the live
  URL.
